            ...
            <div class="dashboard-card">
                ...
                <div class="page-head">
                    <div>
                        ...
                        <p class="lead" style="margin-top:8px;">Draft, optimize, and schedule professional content for your storefront audience.</p>
                    </div>
                ...
                <form method="POST" action="{{ route('admin.blog.posts.store') }}" enctype="multipart/form-data" class="split-grid admin-split-layout" data-auto-slug-form>
                    @csrf
                    ...
                    <div class="section-grid">
                        <section class="panel">
                            ...
                            <div class="field">
                                ...
                                <input name="title" value="{{ old('title') }}" required placeholder="e.g. 5 Benefits of Pure Brass Utensils in Your Kitchen" data-slug-source id="editor-title" />
                            </div>

                            <div class="field">
                                <label>Slug (SEO URL Segment) <span class="text-danger">*</span></label>
                                <input name="slug" value="{{ old('slug') }}" placeholder="e.g. benefits-pure-brass-utensils-kitchen" data-slug-target id="editor-slug" />
                            </div>
                                ...
                                <div class="field">
                                    ...
                                    <select name="blog_author_id" required>
                                        ...
                                        @foreach($authors as $author)
                                            <option value="{{ $author->id }}" @selected(old('blog_author_id') == $author->id)>{{ $author->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                    ...
                                    @foreach($tags as $tag)
                                        <div class="col-md-4 col-sm-6">
                                            <label class="checkbox-row compact">
                                                <input type="checkbox" name="tags[]" value="{{ $tag->id }}" @checked(is_array(old('tags')) && in_array($tag->id, old('tags')))>
                                                <span>{{ $tag->name }}</span>
                                            </label>
                                        </div>
                                    @endforeach
                            ...
                            <div class="field">
                                ...
                                <textarea name="content" id="editor-content" class="code" required placeholder="Write your long-form article content here in rich formatting or clean HTML structure...">{{ old('content') }}</textarea>
                            </div>
                        </section>
                                ...
                                <div class="row g-2 p-3 border rounded" style="max-height: 180px; overflow-y: auto; background: #fff;">
                                    @foreach($products as $product)
                                        <div class="col-md-6 col-sm-12">
                                            <label class="checkbox-row compact">
                                                <input type="checkbox" name="related_products[]" value="{{ $product->id }}" @checked(is_array(old('related_products')) && in_array($product->id, old('related_products')))>
                    ...
                    <div class="section-grid" style="position: sticky; top: 24px;">
                        ...
                        <section class="panel border-primary" style="background: radial-gradient(circle at bottom right, rgba(37, 99, 235, 0.02), transparent);">
                            ...
                            <div class="field">
                                ...
                                <select name="status" id="editor-status" required>
                                    <option value="draft" @selected(old('status') === 'draft')>Draft (Hidden from Store)</option>
                                    <option value="scheduled" @selected(old('status') === 'scheduled')>Scheduled (Publish Future Time)</option>
                                    <option value="published" @selected(old('status') === 'published')>Published (Live Instantly)</option>
                                </select>
                            </div>

                            <div class="field" id="schedule-date-wrap" style="display: none;">
                                <label>Schedule Publish Date & Time</label>
                                <input type="datetime-local" name="published_at" value="{{ old('published_at') }}" id="editor-published-at" />
                            </div>
                        ...
                        <section class="panel">
                            ...
                            <div class="field">
                                ...
                                <input name="primary_keyword" value="{{ old('primary_keyword') }}" id="editor-primary-keyword" placeholder="e.g. brass pooja room" />
                            </div>

                            <div class="field">
                                <label>Secondary Keywords (Comma-separated)</label>
                                <input name="secondary_keywords" value="{{ old('secondary_keywords') }}" placeholder="e.g. brass ganesha, home mandir design, pooja space" />
                            </div>

                            <div class="field">
                                <label>Brief Excerpt Snippet <span class="text-danger" id="excerpt-req-star" style="display:none;">*</span></label>
                                <textarea name="excerpt" id="editor-excerpt" placeholder="Write a short summary (140-180 characters) to show on catalog listings..." style="min-height: 80px;">{{ old('excerpt') }}</textarea>
                            </div>
                            ...
                            <div class="field">
                                ...
                                <input name="meta_title" value="{{ old('meta_title') }}" id="editor-meta-title" placeholder="Keep within 45-65 characters..." />
                            ...
                            <div class="field">
                                ...
                                <textarea name="meta_description" id="editor-meta-desc" placeholder="Keep within 140-160 characters..." style="min-height: 80px;">{{ old('meta_description') }}</textarea>
                            ...
                            <div class="field">
                                ...
                                <input name="featured_image_alt" value="{{ old('featured_image_alt') }}" id="editor-image-alt" placeholder="e.g. Generational handcrafted brass spice box setup" />
                            </div>

                            <div class="field">
                                <label>Canonical Reference URL</label>
                                <input type="url" name="canonical_url" value="{{ old('canonical_url') }}" placeholder="e.g. https://littledivinity.com/blog/..." />
                            </div>

                            <div class="field">
                                <label>Schema LD Structure type</label>
                                <select name="schema_type">
                                    <option value="BlogPosting" @selected(old('schema_type') === 'BlogPosting')>BlogPosting (Standard Blog)</option>
                                    <option value="Article" @selected(old('schema_type') === 'Article')>Article (General Essay)</option>
                                    <option value="NewsArticle" @selected(old('schema_type') === 'NewsArticle')>NewsArticle (Announcements)</option>
                                </select>
                            </div>

                            <div class="field d-flex gap-4 p-2 bg-light border rounded">
                                <label class="checkbox-row compact mb-0">
                                    <input type="checkbox" name="seo_noindex" value="1" @checked(old('seo_noindex'))>
                                    <span>noindex</span>
                                </label>
                                <label class="checkbox-row compact mb-0">
                                    <input type="checkbox" name="seo_nofollow" value="1" @checked(old('seo_nofollow'))>
                                    <span>nofollow</span>
                                </label>
                            </div>
                        </section>
