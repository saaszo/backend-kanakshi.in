...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $authFromEmail = env('CUSTOMER_AUTH_FROM_EMAIL', 'noreply@littledivinity.com');
        $orderFromEmail = env('CUSTOMER_ORDER_FROM_EMAIL', 'order@littledivinity.com');
        ...
        DB::table('customer_email_settings')->updateOrInsert(
            ['id' => 1],
            [
                'from_name' => filled($existing?->from_name ?? null) ? $existing->from_name : env('CUSTOMER_AUTH_FROM_NAME', 'Little Divinity'),
                'from_email' => filled($existing?->from_email ?? null) ? $existing->from_email : $authFromEmail,
                'reply_to_email' => filled($existing?->reply_to_email ?? null) ? $existing->reply_to_email : env('CUSTOMER_AUTH_REPLY_TO_EMAIL', $authFromEmail),
                ...
                'order_from_name' => filled($existing?->order_from_name ?? null) ? $existing->order_from_name : env('CUSTOMER_ORDER_FROM_NAME', 'Little Divinity Orders'),
                'order_from_email' => filled($existing?->order_from_email ?? null) ? $existing->order_from_email : $orderFromEmail,
                'order_reply_to_email' => filled($existing?->order_reply_to_email ?? null) ? $existing->order_reply_to_email : env('CUSTOMER_ORDER_REPLY_TO_EMAIL', $orderFromEmail),
                'order_smtp_username' => filled($existing?->order_smtp_username ?? null) ? $existing->order_smtp_username : env('CUSTOMER_ORDER_SMTP_USERNAME', $orderFromEmail),
